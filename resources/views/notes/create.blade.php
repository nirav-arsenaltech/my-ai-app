...
<a href="{{ route('dashboard') }}">Dashboard</a>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="breadcrumb-separator"><polyline points="9 18 15 12 9 6"/></svg>
    <span class="breadcrumb-active">New</span>
@endsection

@section('page-title', 'Create New Note')
@section('page-subtitle', 'Draft your thoughts and ideas in a secure environment.')

@section('content')
    <div class="editor-container">
        <form action="{{ route('notes.store') }}" method="POST" class="editor-form">
            @csrf
            <div class="admin-card editor-card">
                <div class="editor-header">
                    <input type="text" name="title" id="title" class="editor-title-input @error('title') is-invalid @enderror" 
                           placeholder="Note Title" value="{{ old('title') }}" autofocus>
                    @error('title')
                ...
                <div class="editor-body">
                    <textarea name="content" id="content" class="editor-textarea @error('content') is-invalid @enderror" 
                              ...
                              maxlength="{{ \App\Models\Note::MAX_CONTENT_LENGTH }}">{{ old('content') }}</textarea>
                    @error('content')
                        <div class="field-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="editor-footer">
                    <div class="char-count-wrapper">
                        <span id="char-count">0</span> / {{ \App\Models\Note::MAX_CONTENT_LENGTH }} characters
                        <p id="limit-warning" class="text-danger text-xs mt-1 hidden font-bold">Character limit reached!</p>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <a href="{{ route('notes.index') }}" class="btn btn-secondary">Discard</a>
                        <button type="submit" class="btn btn-primary px-8">Save Note</button>
                    </div>
                </div>
            </div>
        </form>
